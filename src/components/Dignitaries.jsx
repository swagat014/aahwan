...
import { UserCheck, Award, Crown, Shield, Star, Users } from 'lucide-react';
import { dignitariesData } from '../data/scheduleData';
...
export default function Dignitaries() {
  ...
  return (
    <section id="dignitaries" className="py-28 bg-slate-50/80 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          <p className="text-slate-600 text-lg">
            Guiding athletic excellence, university representation, and tournament governance at Government College of Engineering Kalahandi.
          </p>
        </motion.div>
        ...
        <div className="mb-20">
          ...
          >
            {dignitariesData.tier1?.map((leader, idx) => (
              <motion.div
                key={idx}
                variants={cardVariants}
                whileHover={{ y: -8, scale: 1.02 }}
                className="award-glass-card rounded-3xl p-8 text-center border-2 border-amber-300/80 shadow-xl transition-all flex flex-col items-center group relative overflow-hidden"
              >
        ...
        <div className="mb-20">
          ...
          <motion.div
            ...
            className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto"
          >
            {dignitariesData.tier2?.map((leader, idx) => (
              <motion.div
                key={idx}
                variants={cardVariants}
                whileHover={{ y: -8, scale: 1.02 }}
                className="award-glass-card rounded-3xl p-7 text-center border border-slate-200/90 shadow-md hover:shadow-xl transition-all flex flex-col items-center group"
              >
        ...
        <div>
          ...
          <motion.div
            ...
            className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto"
          >
            {dignitariesData.tier3?.map((leader, idx) => (
              <motion.div
                key={idx}
                variants={cardVariants}
                whileHover={{ y: -8, scale: 1.02 }}
